...
const PrintDialog = ({ open, onClose, data }) => {
  ...
  const handleDownload = async () => {
    const input = document.getElementById("print-content"); // Get the content to capture
    ...
    const imgWidth = 210 - 20; // A4 width in mm minus margin (10px margin on each side)
    const pageHeight = 295 - 20; // A4 height in mm minus margin (10px margin on each side)
    const imgHeight = (canvas.height * imgWidth) / canvas.width;
    let heightLeft = imgHeight;
    let position = 10; // Margin top

    pdf.addImage(imgData, "PNG", 10, position, imgWidth, imgHeight); // Margin left
    heightLeft -= pageHeight;

    while (heightLeft >= 0) {
      position = heightLeft - imgHeight;
      pdf.addPage();
      pdf.addImage(imgData, "PNG", 10, position + 10, imgWidth, imgHeight); // Margin left and top
      heightLeft -= pageHeight;
    }
